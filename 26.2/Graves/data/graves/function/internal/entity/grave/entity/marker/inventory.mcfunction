...
$execute if score xp graves.config matches 1 store result score @s graves.marker.xp as @p[scores={utils.player.id=$(player_id)}] run function graves:internal/playerxp/get_total_points