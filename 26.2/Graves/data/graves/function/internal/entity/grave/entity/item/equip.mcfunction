...
$execute if score auto_equip graves.config matches 1 unless data entity @p[scores={utils.player.id=$(player_id)}] $(check) as @n[type=minecraft:item,nbt={Age:0s},tag=graves.grave.item] run function graves:internal/entity/grave/entity/item/auto_equip {"player_id":$(player_id),"slot":"$(slot)"}
